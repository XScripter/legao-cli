...
module.exports = function (done) {
  ...
  if (minNodeVersion > currentNodeVersion) {
    return console.log(chalk.red(
      '  You must upgrade node to >=' + minNodeVersion + '.x to use legao-cli'
    ))
  }

  request({
    url: 'https://registry.npmjs.org/legao-cli',
    timeout: 1000
    ...
    if (!err && res.statusCode === 200) {
      ...
      if (semver.lt(localVersion, latestVersion)) {
        console.log(chalk.yellow('  A newer version of legao-cli is available.'))
        console.log()
